...
/**
   ...
   @return void
*/
Workspace::Workspace(TString newConfigFile, TString newOptions) {
  ...
  if (m_config->getBool("Verbose")) {
    std::cout << "Workspace: Printing config file" << std::endl;
    m_config->printDB();
  }
  ...
  m_outputDir = Form("%s/%s/Workspace", 
  ...
  if (fitsAllConverged()) {
    std::cout << "Workspace: Successfully initialized!" << std::endl;
  }
  else {
    std::cout << "Workspace: Fit failure during initialization." << std::endl;
  }
  return;
}
...
/**
   ...
   this method.
*/
void Workspace::addCategory() {
  printer("Workspace::addCategory()", false);
  ...
  printer("Workspace::addCategory(): Adding non-syst. variables...", false);
  ...
  //--------------------------------------//
  // Add expressions:
  printer("Workspace::addCategory(): Adding expressions...", false);
  ...
  //--------------------------------------//
  // Add component PDFs:
  printer("Workspace::addCategory(): Adding component PDFs...", false);
  ...
  printer("Workspace::addCategory(): Building complete model...", false);
  ...
  printer("Workspace::addCategory(): Importing observed dataset...", false);
  /*
  ...
  if (massInput.is_open()) {
    printer(Form("Workspace: Loading data points from file %s",
  ...
  else printer(Form("Workspace: ERROR! nonexistent input %s", 
		    textFileName.Data()), true);
  massInput.close();
  
  // Import RooDataSet into workspace, and add to dataset map (for combination):
  printer(Form("Workspace: Dataset in %s has %f entries",
  ...
  */
  printer(Form("Workspace: Added cate. %s", m_currCateName.Data()), false);
}
...
*/
void Workspace::addSystematic(TString systematicForm) {
  printer(Form("Workspace::addSystematic(%s)",systematicForm.Data()), false);
  ...
  if ((centralValue.EqualTo("") || constraint.EqualTo("") || 
    ...
    printer(Form("Workspace: ERROR loading syst. %s", systematicForm.Data()),
	    true);
  }
  ...
  if (constraint.EqualTo("asym") && (int)componentsLo.size() == 0) {
    printer("Workspace: Implementing an asymmetric uncertainty with only the"
	...
	std::cout << "Workspace: not prepared for this eventuality" 
		  << std::endl;
	exit(0);
      }
  ...
  if (m_config->getBool("Verbose")) {
    std::cout << "Workspace: Printing " << systematicName 
  ...
  for (std::map<TString,TString>::iterator iterComponent = components.begin();
    ...
    if (constraint.EqualTo("asym")) {
      printer(Form("Workspace: %s asym constraint",systematicName.Data()),
	      false);
	...
	printer(Form("Workspace: %s gaus constraint", systematicName.Data()),
		false);
	...
	printer(Form("Workspace: %s logn constraint",systematicName.Data()),
		false);
	...
	m_ws->factory(Form("prod::%s(nominal_%s,uncert_%s)", expName.Data(),
      ...
      else printer(Form("Workspace: Constraint type ERROR %s",
...
void Workspace::createAsimovData(int valPoI) {
  printer(Form("Workspace::createAsimovData(%d) in category %s", 
  ...
  printer(Form("Workspace: Asimov data entries=%f",currAsimov->sumEntries()),
	  false); 
...
*/
void Workspace::createStupidAsimovData(int valPoI, int nBinsAsimov) {
  printer(Form("Workspace::createStupidAsimovData(%d) in category %s", 
	       valPoI, m_currCateName.Data()), false);
...
/**
   ...
   hypotheses.
*/
void Workspace::createNewWS() {
  printer("Workspace::createNewWS()", false);
  ...
  printer("Workspace: Looping over categories to define workspace.", false);
  ...
  printer("Workspace: Beginning to combine all categories.", false);
...
CHANGE
  ...
  */  
  ...
  //m_modelConfig->SetPdf((*m_ws->pdf("combinedPdf")));CHANGE
  m_modelConfig->SetPdf((*m_ws->pdf("combinedPdfSB")));
  ...
  //----------------------------------------//
  // Create Asimov data:
  printer("Workspace: Perform b-only fit before Asimov creation", false);
  ...
  */
  //----------------------------------------//
  // Save information
  printer("Workspace: Saving workspace.", false);
...
CHANGE
  ...
  std::cout << "Workspace: Printing likelihood results." << std::endl;
  ...
  */
  ...
  TString outputFileName = Form("%s/rootfiles/workspace_%s.root",
				m_outputDir.Data(), m_anaType.Data());
  m_ws->writeToFile(outputFileName);
  printer(Form("Workspace: Workspace saved to %s",outputFileName.Data()),false);
...
*/
bool Workspace::fitsAllConverged() {
  printer("Workspace::fitsAllConverge()", false);
...
*/
RooWorkspace* Workspace::getCombinedWorkspace() {
  printer("Workspace::getCombinedWorkspace()", false);
  return m_ws;
}
...
*/
ModelConfig* Workspace::getModelConfig() {
  printer("Workspace::getModelConfig()", false);
...
/**
   ...
   workspace pointers).
*/
void Workspace::loadWSFromFile() {
  printer("Workspace::loadWSFromFile()", false);
...
*/
TString Workspace::nameOfVar(TString varForm) {
...
*/
TString Workspace::nameOfFunc(TString funcForm) {
...
*/
void Workspace::plotCatePdfAndData(int nBins) {
  printer(Form("Workspace::plotCatePdfAndData(%d)",nBins), false);
...
*/
void Workspace::printer(TString statement, bool isFatal) {
...
*/
void Workspace::setParameters(RooArgSet *set, bool isConstant, double value){
  printer(Form("Workspace::setParameters(%d, %2.2f)",(int)isConstant,value),
	  false);
  RooRealVar *arg = NULL;
