...
//check inputs
  ...
  /*
  if(m_options.Contains("setval")){
    vector<TString> strArr=CommonFunc::SplitString(additionStr,',');
    for(TString item : strArr){
      vector<TString> subStrArr=CommonFunc::SplitString(item,'=');
      TString varName=subStrArr[0];
      double varValue=subStrArr[1].Atof();
      cout<<"\t REGTEST: set "<<varName<<" to "<<varValue<<" and fix it to constant"<<endl;
      statistics::setVal(w->var(varName),varValue, true);
    }
  }
  ...
  */
  ...
  TString rootFileName = Form("%s/text_limits_%s.root", folder.c_str(),
			      saveName.Data());
...
/**
   ...
   @param options - Job options, including "SetVal", ...
   @param 
*/
int main(int argc, char* argv[]) {
  ...
  // WHAT IS THIS? ALSO, HOW DOES IT WORK? HOW DO WE SET MASS< WIDTH?
  /*
  if (m_options.Contains("setval")) {
    if (argc <= 4) abort();
    additionStr=argv[4];
